...
export async function GET(request: Request) {
  ...
  const items = await Education.find(admin ? {} : { visible: true }).sort({ order: 1 }).lean();
...
export async function POST(request: Request) {
  ...
  await logAuditEvent({ action: "educationcreate", entityType: "Education", entityId: String(item._id), success: true });
