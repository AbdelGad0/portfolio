...
export async function GET(request: Request) {
  ...
  const items = await Certification.find(admin ? {} : { visible: true }).sort({ order: 1 }).lean();
...
export async function POST(request: Request) {
  ...
  await logAuditEvent({ action: "certificationcreate", entityType: "Certification", entityId: String(item._id), success: true });
